...
def stage1_audit(file_maps: dict, y0: int, y1: int):
    ...
    for name, (files, native) in file_maps.items():
        if not files:
            ...
            print(
                f"  {name:>5}: NO FILES FOUND  <-- variable would be silently skipped!"
            )
            continue
        ...
        for f in files:
            ...
            per_file.append((f.name, len(secs), str(t.min())[:16], str(t.max())[:16]))
        ...
        print(
            f"  {name:>5}: files={len(files):<4} in-range-steps={len(all_ts):<7}{flag}"
        )
        for fn, n, lo, hi in (
            per_file[:3]
            + ([("...", "", "", "")] if len(per_file) > 6 else [])
            + per_file[-3:]
            if len(per_file) > 6
            else per_file
        ):
...
def stage2_scan(file_maps: dict, files_per_var: int, problems: list):
    ...
    for name, (files, native) in file_maps.items():
        if not files:
            continue
        idx = np.unique(
            np.linspace(0, len(files) - 1, min(files_per_var, len(files)))
            .round()
            .astype(int)
        )
        ...
        print(
            f"  {name:>5} ({native:>8}): scanned {len(idx)} files, {n_tot / 1e6:.0f}M vals | "
...
def stage3_dataset(args, problems: list):
    print("\n" + "=" * 78)
    print(
        f"STAGE 3 - DATASET-PATH SAMPLE CHECK ({args.n_samples} samples via LANLMJODataset)"
    )
    print("=" * 78)
    import torch
    ...
    idx = np.unique(
        np.linspace(0, len(ds) - 1, min(args.n_samples, len(ds))).round().astype(int)
    )
    ...
    for i in idx:
        ...
        groups = [
            ("in.surf", in_batch.surf_vars),
            ("in.atmos", in_batch.atmos_vars),
            ("tgt.surf", surf_t[0]),
            ("tgt.atmos", atmos_t[0]),
        ]
    ...
    for key in sorted(stats):
        lo, hi, nf = stats[key]
        flag = (
            f"  <-- {nf} NON-FINITE (should be impossible post-process_var!)"
            if nf
            else ""
        )
...
def stage4_forward(args, problems: list):
    ...
    for attr in ("surf_vars", "atmos_vars"):
        for name, p in getattr(pred, attr).items():
            ...
            print(
                f"  {attr[:-5]:>5}.{name:>5}: pred_range=[{p2.min():.4g},{p2.max():.4g}] "
...
def main():
    ...
    root = Path(args.root)
    fmap = {
        **_collect(SURFACE_VAR_MAP, root, *args.years),
    ...
    if problems:
        ...
        print(
            "  - out-of-range timesteps        -> leakage / glob over-match (fixed by v3 dataset)"
        )
        print(
            "  - timestamp set differs         -> hypothesis B confirmed (fixed by v3 dataset)"
        )
        print(
            "  - NaN/Inf/extremes in raw scan  -> hypothesis A confirmed (v3 zeroes them; consider"
        )
        print(
            "                                     excluding affected files/years from val)"
        )
        print("  - non-finite only at stage 4    -> model/normalization-side, not data")
        sys.exit(1)
    print(
        "VERDICT: CLEAN - no leakage, no misalignment, no bad values in the scanned range."
    )
    sys.exit(0)

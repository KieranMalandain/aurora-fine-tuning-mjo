...
def explore_data():
    ...
    for name, dir_hints, pattern in targets:
        ...
        if not found_file:
            print(
                f"[MISSING] Could not find any files for {name} using hints {dir_hints} and pattern {pattern}"
            )
        ...
        try:
            ...
            if data_vars:
                for v in data_vars:
                    ...
                    plev_coords = [
                        c for c in ["lev", "isobaricInhPa", "level"] if c in ds.coords
                    ]
                    if plev_coords and plev_coords[0] in ds[v].dims:
                        levels = ds[plev_coords[0]].values
                        print(
                            f"  Pressure levels ({len(levels)}): {levels[:5]} ... {levels[-5:]}"
                        )
            else:
    ...
    print("=" * 110)
    print(
        f"{'Variable':<15} | {'Status':<9} | {'Resolved Directory Path':<50} | {'Native NetCDF Vars'}"
    )
    print("-" * 110)
    for name, status, rel_dir, vars_found in report:
        if status == "FOUND":
            v_str = ", ".join(vars_found) if vars_found else "None"
            print(f"{name:<15} | {status:<9} | {rel_dir!s:<50} | {v_str}")
        else:
